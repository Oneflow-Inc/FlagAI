...
class SwinTransformerV2(BaseModel):
    ...
    def __init__(self, config, num_classes=1000,  **kwargs):
        ...
        self.patch_embed = PatchEmbed(
            img_size=img_size, patch_size=swin_config.patch_size, in_chans=swin_config.in_chans, embed_dim=embed_dim,
            norm_layer=swin_config.norm_layer if self.patch_norm else None)
        ...
        # stochastic depth
        dpr = [x.item() for x in torch.linspace(0, swin_config.drop_path_rate, sum(depths))]  # stochastic depth decay rule
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               ...
                               num_heads=swin_config.num_heads[i_layer],
                               window_size=swin_config.window_size,
                               mlp_ratio=self.mlp_ratio,
                               qkv_bias=swin_config.qkv_bias,
                               drop=drop_rate, attn_drop=swin_config.attn_drop_rate,
                               drop_path=dpr[sum(depths[:i_layer]):sum(depths[:i_layer + 1])],
                               norm_layer=swin_config.norm_layer,
                               downsample=PatchMerging if (i_layer < self.num_layers - 1) else None,
                               use_checkpoint=swin_config.checkpoint_activations,
                               pretrained_window_size=swin_config.pretrained_window_sizes[i_layer])
            self.layers.append(layer)

        self.norm = swin_config.norm_layer(self.num_features)
